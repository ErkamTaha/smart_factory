...
class UserMQTTClient:
    ...
    async def publish(
        ...
        # Check sensor limits for alert
        parts = topic.split("/")
        if len(parts) >= 3:
            sensor_id = parts[-1]

        sensor_value = payload["value"]
        sensor_unit = payload["unit"]

        alert, alert_type = self._check_ss_limit(sensor_id, sensor_value, sensor_unit)
        alert_message = "Sensor {sensor_id} exceeded {alert_type} limit with value: {sensor_value} and unit: {sensor_unit}."
